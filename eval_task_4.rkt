...
; Added (Kuzmichev)
; *************** Added ***************
(define (make-define-func name vars exp)
  (append '(define) (append (list (append name vars)) exp)))
; *************** Added ***************
...
(define (until-body exp) 
  (define (find-body exp)
    (if (null? (cdr exp))
        '()
        (cons (car exp) (find-body (cdr exp)))))
  (cdr (find-body exp)))
; *************** Added ***************
...
; Added (Kuzmichev)
; *************** Added ***************
(define (until->transformed until-exp)
  (let* ([test (until-test until-exp)]
         [body (until-body until-exp)]
         [if-statement (make-if test '(void) '(loop))])

        (make-let '() (list (make-define-func '(loop) '() (append body (list if-statement))) '(loop)))))
; *************** Added ***************

; 99 bottles test
; Execute: (meval (bottles-test))
